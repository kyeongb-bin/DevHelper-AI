...
export const ErrorAnalysisResult: React.FC<
    ErrorAnalysisResultProps
> = ({ data, onRegenerate }) => {
    ...
    const handleCopy = async (
        text: string,
        type: 'summary' | 'solution'
    ) => {
        if (copiedType === type) return; // 이미 복사된 경우 무시
            ...
            <div className='p-4 border border-orange-200 dark:border-orange-800 rounded-lg bg-orange-50 dark:bg-orange-900/20'>
                <div className='flex items-start justify-between gap-4'>
                    ...
                    <button
                        ...
                        disabled={copiedType === 'summary'}
                        className={`px-3 py-1 text-xs rounded transition-all flex items-center gap-1.5 flex-shrink-0 ${
                            copiedType === 'summary'
